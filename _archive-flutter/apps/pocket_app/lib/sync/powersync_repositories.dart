...
class PowerSyncAccountRepository implements AccountRepository {
  ...
  /// All accounts, archived included — consumers filter via
  /// `activeAccountsProvider` (the Accounts screen shows an Archived section).
  @override
  Stream<List<LedgerAccount>> watch() => db
      .watch('SELECT * FROM accounts ORDER BY name')
...
class PowerSyncAccountGroupRepository implements AccountGroupRepository {
  final PowerSyncDatabase db;
  PowerSyncAccountGroupRepository(this.db);

  @override
  Stream<List<AccountGroup>> watch() => db
      .watch('SELECT * FROM account_groups ORDER BY sort_order, name')
      .map((rs) => rs.map((r) => accountGroupFromRow(_asMap(r))).toList());

  @override
  Future<void> upsert(AccountGroup g) async {
    final r = accountGroupToRow(g);
    await db.execute(
      '''INSERT OR REPLACE INTO account_groups
         (id, user_id, name, sort_order)
         VALUES (?, ?, ?, ?)''',
      [r['id'], _currentUserId(), r['name'], r['sort_order']],
    );
  }

  @override
  Future<void> remove(String id) =>
      db.execute('DELETE FROM account_groups WHERE id = ?', [id]);
}
    ...
    await db.execute(
      '''INSERT OR REPLACE INTO transactions
         (id, user_id, account_id, category_id, type, amount_minor, currency,
          ...
          debt_id, debt_role, regular_item_id, recurring, recurring_source_id)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
      [
        ...
        r['recurring'] == null ? null : jsonEncode(r['recurring']),
        r['recurring_source_id'],
      ],
    );
    ...
    for (final s in t.splits ?? const []) {
      ...
      await db.execute(
        '''INSERT OR REPLACE INTO transaction_splits
           ...
           VALUES (?,?,?,?,?,?)''',
        // Fresh splits from the UI have no row id yet — PowerSync requires one.
        [sr['id'] ?? const Uuid().v4(), sr['transaction_id'], sr['account_id'],
         sr['category_id'], sr['amount_minor'], sr['acct_minor']],
      );
    }
  }
...
/// Remembered payee→category mappings backed by `merchant_categories`. The
/// Postgres PK is a uuid, so the row id is a deterministic UUIDv5 of
/// (user, merchant): re-saving the same merchant updates one row instead of
/// piling up duplicates, and stays valid for the Supabase upload.
class PowerSyncMerchantCategoryRepository
    implements MerchantCategoryRepository {
  final PowerSyncDatabase db;
  PowerSyncMerchantCategoryRepository(this.db);

  String _id(String merchant) => const Uuid().v5(
      Namespace.url.value, 'pocket:merchant:${_currentUserId()}:$merchant');

  @override
  Stream<List<MerchantCategory>> watch() => db
      .watch('SELECT * FROM merchant_categories')
      .map((rs) => rs.map((r) => merchantCategoryFromRow(_asMap(r))).toList());

  @override
  Future<void> upsert(MerchantCategory m) => db.execute(
        '''INSERT OR REPLACE INTO merchant_categories
           (id, user_id, merchant, category_id)
           VALUES (?, ?, ?, ?)''',
        [_id(m.merchant), _currentUserId(), m.merchant, m.categoryId],
      );

  @override
  Future<void> remove(String merchant) => db.execute(
      'DELETE FROM merchant_categories WHERE id = ?', [_id(merchant)]);
}
