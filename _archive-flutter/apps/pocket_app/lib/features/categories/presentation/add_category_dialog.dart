...
class _CategoryDialogState extends ConsumerState<CategoryDialog> {
  ...
  Future<void> _save() async {
    ...
    await ref.read(categoryControllerProvider).upsert(category);
  ...
  Future<void> _delete() async {
    ...
    setState(() => _busy = true);
    await ref.read(categoryControllerProvider).delete(
          category: c,
          allCategories: cats,
          allTransactions:
              ref.read(transactionsProvider).valueOrNull ?? const [],
          allBudgets: ref.read(budgetsProvider).valueOrNull ?? const [],
        );
    if (mounted) Navigator.of(context).pop();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final parents = cats
        ...
        .toList();
    // Stored parent may no longer qualify (deleted, re-typed, demoted) —
    // sanitize so the dropdown never gets an unknown value.
    final parentValue =
