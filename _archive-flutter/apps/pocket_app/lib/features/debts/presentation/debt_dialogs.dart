      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // On edit the date taken is frozen (it stamps the initial
            // transaction) but shown read-only, like the legacy modal.
            if (editing != null)
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('${_dateTaken.year.toString().padLeft(4, '0')}-'
                        '${_dateTaken.month.toString().padLeft(2, '0')}-'
                        '${_dateTaken.day.toString().padLeft(2, '0')}'),
                  ],
                ),
              ),
            _DateRow(
...
class _DebtPaymentDialogState extends ConsumerState<DebtPaymentDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    // Linked account may be archived/deleted — never feed a dropdown a value
    // that isn't among its items (Flutter asserts).
    final accountValue = accounts.any((a) => a.id == _accountId)
        ? _accountId
        : (accounts.isNotEmpty ? accounts.first.id : null);
    if (!_prefilled) {
      _prefilled = true;
