...
abstract final class LedgerMath {
  ...
  /// Home-currency value of a transaction for AGGREGATES (dashboard totals,
  /// reports): prefers the rate-frozen write-time snapshot
  /// ([LedgerTransaction.refAmountMinor]) so history never drifts when FX
  /// rates refresh; falls back to a live conversion for rows that predate
  /// the snapshot. NOTE: snapshots are denominated in the home currency
  /// that was active at write time — changing the home currency leaves old
  /// snapshots in the previous home (legacy behaved the same way).
  static int homeAmount(
          LedgerTransaction tx, CurrencyService fx, String home) =>
      tx.refAmountMinor ?? fx.convert(tx.amount, tx.currency, home);
