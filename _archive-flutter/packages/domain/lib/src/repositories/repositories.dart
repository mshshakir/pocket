...
import '../accounts/account_group.dart';
import '../ledger/ledger_entities.dart';
import '../merchant/merchant_category.dart';
...
abstract interface class AccountGroupRepository {
  Stream<List<AccountGroup>> watch();
  Future<void> upsert(AccountGroup group);
  Future<void> remove(String id);
}
...
/// Remembered payee→category mappings (auto-categorisation). [remove] is keyed
/// by the normalised merchant string, not a synthetic id.
abstract interface class MerchantCategoryRepository {
  Stream<List<MerchantCategory>> watch();
  Future<void> upsert(MerchantCategory mapping);
  Future<void> remove(String merchant);
}
