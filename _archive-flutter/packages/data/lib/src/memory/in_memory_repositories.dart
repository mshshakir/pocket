...
/// In-memory [AccountGroupRepository].
class InMemoryAccountGroupRepository implements AccountGroupRepository {
  final List<AccountGroup> _items;
  final _controller = StreamController<List<AccountGroup>>.broadcast();

  InMemoryAccountGroupRepository([List<AccountGroup> seed = const []])
      : _items = [...seed];

  @override
  Stream<List<AccountGroup>> watch() async* {
    yield List.unmodifiable(_items);
    yield* _controller.stream;
  }

  @override
  Future<void> upsert(AccountGroup group) async {
    final i = _items.indexWhere((g) => g.id == group.id);
    if (i >= 0) {
      _items[i] = group;
    } else {
      _items.add(group);
...
/// In-memory [MerchantCategoryRepository] — keyed by the normalised merchant.
class InMemoryMerchantCategoryRepository
    implements MerchantCategoryRepository {
  final List<MerchantCategory> _items;
  final _controller = StreamController<List<MerchantCategory>>.broadcast();

  InMemoryMerchantCategoryRepository([List<MerchantCategory> seed = const []])
      : _items = [...seed];

  @override
  Stream<List<MerchantCategory>> watch() async* {
    yield List.unmodifiable(_items);
    yield* _controller.stream;
  }

  @override
  Future<void> upsert(MerchantCategory mapping) async {
    final i = _items.indexWhere((m) => m.merchant == mapping.merchant);
    if (i >= 0) {
      _items[i] = mapping;
    } else {
      _items.add(mapping);
  ...
  Future<void> remove(String merchant) async {
    _items.removeWhere((m) => m.merchant == merchant);
    _emit();
  }
