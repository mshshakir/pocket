...
// ── account groups ─────────────────────────────────────────────────────
Row accountGroupToRow(AccountGroup g) => {
      'id': g.id,
      'name': g.name,
      'sort_order': g.sortOrder,
    };

AccountGroup accountGroupFromRow(Row r) => AccountGroup(
      id: r['id'] as String,
      name: r['name'] as String? ?? '',
      sortOrder: _toInt(r['sort_order']) ?? 0,
    );
...
Row transactionToRow(LedgerTransaction t) => {
      ...
      'recurring': recurringSpecToJson(t.recurring),
...
LedgerTransaction transactionFromRow(Row r, {List<Row> splitRows = const []}) {
  ...
  return LedgerTransaction(
    ...
    recurring: recurringSpecFromJson(r['recurring']),
    recurringSourceId: r['recurring_source_id'] as String?,
    splits: splitRows.isEmpty ? null : splitRows.map(splitFromRow).toList(),
  );
}

// ── recurring spec ↔ jsonb/json-text ───────────────────────────────────
/// Postgres stores this as `jsonb` (arrives as a Map); PowerSync's local
/// SQLite stores it as JSON text. Emit a Map (Supabase + tests); the
/// PowerSync repo stringifies it.
Map<String, dynamic>? recurringSpecToJson(RecurringSpec? s) => s == null
    ? null
    : {
        'rule': s.rule.name,
        'interval': s.interval,
        if (s.until != null) 'until': _dateToIso(s.until),
      };

RecurringSpec? recurringSpecFromJson(Object? v) {
  Map<String, dynamic>? m;
  if (v is Map) m = Map<String, dynamic>.from(v);
  ...
  final rule = switch (m['rule']) {
    'daily' => RecurrenceRule.daily,
    'weekly' => RecurrenceRule.weekly,
    'yearly' => RecurrenceRule.yearly,
    _ => RecurrenceRule.monthly,
  };
  return RecurringSpec(
    rule: rule,
    interval: _toInt(m['interval']) ?? 1,
    until: _dateFromIso(m['until'] as String?),
  );
}
...
// ── merchant categories (auto-categorisation) ──────────────────────────
Row merchantCategoryToRow(MerchantCategory m) => {
      'merchant': m.merchant,
      'category_id': m.categoryId,
    };

MerchantCategory merchantCategoryFromRow(Row r) => MerchantCategory(
