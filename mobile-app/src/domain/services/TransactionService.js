...
export class TransactionService {
  ...
  delete(id) {
    ...
    // A generated recurring instance must be remembered as skipped, or the
    // next process() pass regenerates it from the same deterministic id.
  ...
  bulkDelete(ids) {
    ...
    // Same skip bookkeeping as delete(), for every selected instance.
    const recurring = new RecurringService();
    state.transactions.forEach((t) => { if (idSet.has(t.id)) recurring.recordSkip(t); });
