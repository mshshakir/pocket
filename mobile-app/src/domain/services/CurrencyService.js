...
export class CurrencyService {
  ...
  formatMoney(minor, currency, opts = {}) {
    ...
    try {
      ...
      const sign = value < 0 ? '-' : '';
      // Intl rejected the code, so it isn't a real currency. Echoing it back
      // verbatim would carry arbitrary text (from a CSV import, or another
      // user's shared snapshot) straight into the caller's HTML — this string
      // is interpolated into innerHTML by every view. Only an ISO-4217-shaped
      // code is passed through.
      const safe = CurrencyService.isIsoCode(currency) ? currency : '?';
