...
8. If you cannot make out any amount, set "total" to 0 and "items" to [].`;
   ...
   */
  /**
   * The account a scan/voice prefill should target. Honours the Settings
   * preference, but falls back when it names a deleted or archived account.
   * @param {object} state
  ...
  #defaultAccountId(state) {
    ...
    return (state.accounts.find((a) => !a.archived) || state.accounts[0])?.id;
  }

  /** @returns {string} the Settings default payment method */
  #defaultPaymentType() {
    return this.#store.getState().user?.defaultPaymentType || 'card';
  }
