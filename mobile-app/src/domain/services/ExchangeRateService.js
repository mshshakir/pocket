...
export class ExchangeRateService {
  ...
  async refresh(force = false) {
    ...
    try {
      ...
      // A refreshed rate table is a device-local cache, not user data. It runs
      // on every boot, so pushing it would burn a cloud write (and a version
      // bump for every other device) on data that re-fetches itself anyway;
      // it still rides along with the next genuine push.
