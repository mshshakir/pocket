 ...
 *
 * Where the snapshots come from differs by platform, which is why `sync` is an
 * accepted dependency: the web app injects `state._sharedData` on every render,
 * while the mobile app never populates that key and reads
 * `syncService.sharedData` directly. Taking either lets this file stay a
 * verbatim copy on both sides rather than forking.
 */
import { Store } from '../../core/Store.js';

export class RegularLogService {
  /** @type {Store} */ #store;
  /** @type {object|null} */ #sync;
  ...
  all() {
    ...
    const shared = this.#shares().flatMap((share) =>
      (share.transactions || [])
