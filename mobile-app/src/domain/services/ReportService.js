  ...
  netWorthSeries() {
    ...
    const homeDelta = (t) => {
      ...
      for (const c of LedgerMath.contributions(t)) {
        ...
        if (!acc) continue;
        // Prefer the frozen account-currency impact (acctMinor) that
        // LedgerMath uses for balances. Converting the raw amount live meant
        // the chart's last point drifted away from totalBalanceInHome() after
        // every rate refresh, and manual per-transaction rates were ignored
        // entirely.
        d += Number.isFinite(c.acctMinor)
          ? this.#fx.convert(c.acctMinor, acc.currency, home)
          : this.#fx.convert(Number.isFinite(c.minor) ? c.minor : 0, c.currency, home);
      }
      return d;
    };
